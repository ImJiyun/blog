...
- **Admin** (`/admin/login`, `/admin/posts`, editor): same wordmark/pill language as
  the public site — bordered `--card-bg` cards for login and the post editor, a
  `--radius-sm` bordered table for the post list with `--radius-pill` action buttons,
